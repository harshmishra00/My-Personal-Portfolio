...
const MobileProjectCard = ({ project, index }) => (
  <motion.div
    ...
    className="rounded-[24px] border border-[#e5e5e5] bg-[#f5f5f5] overflow-hidden shadow-lg"
  >
    {/* Image */}
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center gap-2 text-[#999] text-xs font-bold uppercase tracking-widest">
        <span>{project.year}</span><span>•</span><span>{project.category}</span>
      </div>
      <h2 className="text-3xl font-display font-medium text-[#0f0f0f] leading-[0.95]">{project.title}</h2>
      <p className="text-[#555] text-sm leading-relaxed">{project.description}</p>

      {project.techStack && (
        <div className="flex flex-wrap gap-2">
          {project.techStack.map((tech, i) => (
            <span key={i} className="px-3 py-1 bg-white border border-[#e5e5e5] rounded-full text-[#555] text-xs font-medium">
              {tech}
            </span>
          ))}
        </div>
            ...
            <div className="space-y-6">
              ...
              {project.techStack && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {project.techStack.map((tech, i) => (
                    <span key={i} className="px-3 py-1 bg-white border border-[#e5e5e5] rounded-full text-[#555] text-xs font-medium tracking-wide">
                      {tech}
                    </span>
                  ))}
                </div>
